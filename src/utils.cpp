/**
 * utility functions like import export
 */
    ...
    if (!file)
        ...
        if (stat->err.err_num == GML_OK) {
                        ...
                        if (!strcmp(list1->key, "node")) {
                            ...
                            while (list2) {
                                ...
                                if (!strcmp(list2->key, "id")) {
                                    int node_ordinal = g.add_node(
                                            (int) list2->value.integer);
                                    std::pair<long, int> entry(
                                        list2->value.integer, node_ordinal);
                            ...
                            while (list2) {
                                ...
                                if (!strcmp(list2->key, "source")) {
                                    src_node =
                                        id_ordinal_map[list2->value.integer];
                                    //printf("%d\n", list2->value.integer);
            ...
            printf("Error reading line %d column %d of %s:\n", stat->err.line,
                    stat->err.column, filename.c_str());
        }
    }
    return g;
}
