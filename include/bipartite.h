    ...
    while (!fail) {
        while(!nodeque.empty() && !fail) {
            ...
            for (int index = 0; index != successors.size(); index++) {
                ...
                auto finditer =
                    std::find(visitedlist.begin(),
                        visitedlist.end(), successors[index]);
        ...
        if (nodes.size() < g.num_nodes()) {
            for
            (auto node = g.nodes_begin() ; node != g.nodes_end(); node++ ) {
...
std::vector<int> get_maximum_matching(UndirectedGraph<T> g)
{
    ...
    if (bipartitenodes.first().size() > 0)
    {
        ...
        //add edges from source to left half of bipartite nodes
...
template <class T>
std::vector<int>
get_max_flow(UndirectedGraph<T> g, int node_source,
    int node_sink, int** capacity)
{
    ...
    {
        std::vector<int> path =
            get_flow_path(g, node_source, node_sink, capacity);
...
template <class T>
std::vector<int>
get_flow_path(UndirectedGraph<T> g, int node_source,
    int node_sink, int** capacity)
{
    ...
    while(que.size() > 0 && exitwhile == false)
    {
        ...
        for(int succ : succs)
        {
            if(g.get_node_attribute(succ, "visited") != 1 &&
                g.get_node_attribute(where, "capacity") > 0)
            {
                que.push(succ);
