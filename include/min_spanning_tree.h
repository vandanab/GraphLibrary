...
public:
    ...
    QueueNode(int n_o, double w) :
            node_ordinal(n_o), weight(w), parent_node_ordinal(-1), parent_edge(
                    -1) {
    }
...
void minimum_spanning_tree(UndirectedGraph<T> g) {
    ...
    while (!node_queue.empty()) {
        n = node_queue.front();
        pop_heap(node_queue.begin(), node_queue.end());
        node_queue.pop_back();
        ...
        for (auto i = successors.begin(); i != successors.end(); i++) {
            ...
            if (it != node_queue.end()) {
                ...
                if (w < (*it).weight) {
                    ...
                    (*it).weight = w;
                    g.add_node_attribute((*it).node_ordinal, "parent",
                            n.node_ordinal);
                    g.add_node_attribute((*it).node_ordinal, "parent_edge",
                            edge_ord);
                    std::make_heap(node_queue.begin(), node_queue.end());
                }
            }
