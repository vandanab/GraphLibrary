/**
 * graphs.h
 * provides the interface for the basic graph types
 *  - directed
 *  - undirected
 *
...
public:
    ...
    std::vector<int> add_nodes(std::vector<T> nodes_)
    {    return g.add_nodes(nodes_);}

    template<typename... Nodes> std::vector<int>
    add_nodes(T n, Nodes... nodes_) {
        return g.add_nodes(n, nodes_...);
    }

    //returns ordinal(index) of the newly added edge
    int add_edge(int node1_ordinal, int node2_ordinal)
    {    return g.add_edge(node1_ordinal, node2_ordinal);}

    std::vector<int> add_edges(std::vector<std::pair<int, int> > edges_)
    {    return g.add_edges(edges_);}

    template<typename... Edges> std::vector<int>
    add_edges(std::pair<int, int> e, Edges... edges_) {
    ...
    void
    add_node_attribute(int node_ordinal, std::string key, std::string value) {
        g.add_node_attribute(node_ordinal, key, value);
    }
    ...
    get_node_attributes(int node_ordinal) {
    ...
    {return g.neighbors(node_ordinal);}

    std::vector<std::pair<int, int> > neighbors_with_edges(int node_ordinal) {
        return g.neighbors_with_edges(node_ordinal);
    }

    std::vector<int> get_edges(int node_ordinal)
    {return g.get_edges_undirected(node_ordinal);}
...
public:
    ...
    int add_edge(int node1_ordinal, int node2_ordinal)
    {g.add_edge(node1_ordinal, node2_ordinal);}
    ...
    //move constructor should prevent performance issues
    std::vector<int> neighbors(int node_ordinal)
    {return g.successors(node_ordinal);}
    ...
    {return g.predecessors(node_ordinal);}

    std::vector<int> get_edges(int node_ordinal)
    {return g.get_edges_directed(node_ordinal);}

    std::vector<int> in_edges(int node_ordinal)
    {return g.in_edges(node_ordinal);}

    std::vector<int> out_edges(int node_ordinal)
    {return g.out_edges(node_ordinal);}
