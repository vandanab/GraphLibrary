...
std::vector<int> get_shortest_path(T g, int node_source, int node_target) {
	...
	for (; iter != g.nodes_end(); iter++) {
		if (*iter == node_source)
			...
			g.add_node_attribute(*iter, "tentativedistance",
	...
	while (lastvisitednode != node_target && node_queue.size() != 0) {
		...
		auto curr_distance = g.get_node_attribute(curr_node,
				"tentativedistance");
		auto it = curr_succ.begin();
		for (; it != curr_succ.end(); it++) {
			double edge_weight = g.get_edge_attribute(*it.second(), "weight");
			double succ_distance = g.get_node_attribute(*it.first(),
					"tentativedistance");
			bool succ_visited = g.get_node_attribute(*it.first(), "visited");
			if (!succ_visited
					&& (curr_distance + edge_weight) < succ_distance) {
				g.add_node_attribute(*it.first(), "tentativedistance",
						curr_distance + edge_weight);
	...
	//Step 3: Checking if there indeed exists a path.
	//        Else, the target node would have never been visited.
	//        If paths exists, then its the shortest path from node_source.
	//        Hence return it.
	if (lastvisitednode == node_target) {
		curr_parent = -1;
		curr_node = node_target;
...
std::vector<Graph<T> > get_connected_components(T g) {
	...
	while (visitednodecount < g.num_nodes()) {
		...
		//Traverse remaining nodes and
		//remove those that were visited by this BFS
